...
import type { Book, Playlist, PlaylistItem, Progress, Series } from '../types';
...
import { DEFAULT_HOME_LAYOUT, normalizeHomeLayout } from '../utils/homeLayout';
import {
  Calendar,
  ChevronRight,
  Clock,
...
const collectPlaylistCoverCandidates = (playlist: Playlist): CollectionCover[] => {
  const covers: CollectionCover[] = [];
  const pushCover = (cover: CollectionCover) => {
    covers.push(cover);
  };
...
const collectPlaylistCovers = (playlist: Playlist, seed: number): CollectionCover[] => {
  const covers = collectPlaylistCoverCandidates(playlist);
  if (covers.length === 0) return [];
  return [covers[playlistCoverIndex(playlist.id, seed, covers.length)]];
};

const getSeriesCover = (series: Series): CollectionCover[] => {

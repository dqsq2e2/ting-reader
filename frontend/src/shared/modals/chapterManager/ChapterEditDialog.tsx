...
const ChapterEditDialog: React.FC<Props> = ({ chapter, location, onClose, onSave }) => {
  ...
  const [chapterIndex, setChapterIndex] = useState(String(chapter.chapterIndex));
  const [isExtra, setIsExtra] = useState(Boolean(chapter.isExtra));
    ...
    <div className="fixed inset-0 z-[260] flex items-end justify-center bg-slate-950/45 p-0 sm:items-center sm:p-4">
      <div className="w-full max-w-xl rounded-t-3xl bg-white shadow-2xl dark:bg-slate-900 sm:rounded-3xl">
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-slate-800">
          <div>
            <h3 className="text-lg font-bold text-slate-950 dark:text-white">编辑章节</h3>
            <p className="mt-1 text-sm text-slate-500">修改会先暂存，点击保存更改后生效。</p>
          </div>
          <button
            type="button"
            onClick={onClose}

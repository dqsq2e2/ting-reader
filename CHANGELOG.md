...
### 🛠️ 修复与优化
- **修复企业微信机器人对接**：按机器人协议生成 `msgtype` 和 `markdown`/`text` 请求体，并识别 HTTP 200 响应中的非零 `errcode`。
- **模板变量安全转义**：新增 `{{json:变量}}` 语法，自动处理引号、换行和嵌套 JSON，避免模板生成非法请求体。
- **通知配置平滑升级**：数据库升级至 v21，已有 Webhook 自动保留并继续使用原始事件 JSON 格式。
- **FPK 动态端口修复**：移除固定端口检查声明，安装向导配置的端口可正确用于 Docker 与原生 FPK。

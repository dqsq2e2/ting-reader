...
### ✨ 新功能
- **本地存储库授权目录检查与管理**：支持多路径授权本地存储库。管理员可在添加/编辑本地库时输入或浏览通过系统环境变量（如 `TRIM_DATA_ACCESSIBLE_PATHS`）或配置文件授权的绝对路径。
...
### 🛠️ 修复与优化
- **章节管理显示路径优化**：规范化章节列表中本地和远程（如 WebDAV）音频文件的展示路径，过滤 Windows 冗余前缀并统一使用正斜杠，提升路径可读性。

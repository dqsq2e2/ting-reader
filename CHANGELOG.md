...
- **Safari/iOS STRM 自动代理**：Safari 及 iOS 浏览器访问 STRM 文件时，自动走服务端代理（而非 302 重定向），添加 `Accept-Ranges: bytes` 头并转发 Range 请求，解决 Safari 拖动进度条失败的问题，且无转码 CPU 开销。其他浏览器保持 302 重定向（零带宽成本）。

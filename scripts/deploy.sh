...
install_management_commands() {
  ...
  printf '%s\n' "$mode" >"$install_dir/.service-mode"
  id -un >"$install_dir/.service-user"

  if [[ "$mode" == "system" ]]; then
    command_path="/usr/local/bin/ting-reader-library"
    if [[ "$EUID" -eq 0 ]]; then
    ...
    if [[ "$EUID" -eq 0 ]]; then
      ...
      ln -sfn "$install_dir/update.sh" /usr/local/bin/ting-reader-update
    else
      sudo ln -sfn "$install_dir/uninstall.sh" /usr/local/bin/ting-reader-uninstall
      sudo ln -sfn "$install_dir/update.sh" /usr/local/bin/ting-reader-update
    fi
  else
...
UPDATE_URL="${DOWNLOAD_PREFIX}https://github.com/$REPOSITORY/releases/latest/download/$UPDATE_FILE"
...
curl -fL --retry 3 "$UPDATE_URL" -o "$WORK_DIR/$UPDATE_FILE"
...
[[ -s "$WORK_DIR/$UPDATE_FILE" ]] || { say "更新脚本下载失败。" "The update script is missing."; exit 1; }
...
say "交互更新：ting-reader-update" "Interactive update: ting-reader-update"

...
use super::enums::{FormatMethod, ScraperMethod, UtilityMethod};
    ...
    pub async fn call_utility(
        ...
        let instance = {
            ...
            if entry.metadata.plugin_type != crate::plugin::types::PluginType::Utility {
                return Err(TingError::PluginExecutionError(format!(
                    "Plugin {} is not a utility plugin",
                    id
                )));
            }
            entry.instance.clone()
    ...
    pub async fn get_ffmpeg_path(&self) -> Option<String> {
        ...
        if let Some(id) = plugin_id {
            if let Ok(result) = self
                .call_utility(&id, UtilityMethod::GetFfmpegPath, serde_json::json!({}))
                .await
            {
                return result
                    .get("path")

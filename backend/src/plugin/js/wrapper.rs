...
impl JavaScriptPluginWrapper {
    ...
    pub fn new_with_host_gateway(
            ...
            .spawn(move || {
                ...
                let thread_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    ...
                    local.block_on(&rt, async move {
                        ...
                        match tokio::time::timeout(Duration::from_secs(30), executor.load_module())
                            .await
                        {
                            Ok(Ok(())) => {
                        ...
                        while let Some(cmd) = rx.recv().await {
                            match cmd {
                                ...
                                JsCommand::GarbageCollect { resp } => {
                                    let result = executor.garbage_collect().map_err(|e| {
                                        TingError::PluginExecutionError(e.to_string())
                                    });
                ...
                if let Err(panic_err) = thread_result {
                    ...
                    error!(
                        "JS worker thread panicked for {}: {}",
                        plugin_id_clone2, panic_msg
                    );
                }
            });
        ...
        info!(
            "Waiting 3s to check for early initialization errors for {}",
            plugin_id
        );
        ...
        match init_check.join() {
            Ok(recv_result) => {
                match recv_result {
                    Ok(err_msg) => {
                        ...
                        error!(
                            "JS plugin {} failed early initialization check: {}",
                            plugin_id, err_msg
                        );
            ...
            Err(_) => {
                ...
                return Err(TingError::PluginLoadError(
                    "Init check thread panicked".to_string(),
                ));
            }
        }
...
impl Plugin for JavaScriptPluginWrapper {
    ...
    async fn initialize(&self, context: &PluginContext) -> Result<()> {
        ...
        self.tx
            .send(JsCommand::Initialize {
                context: context.clone(),
                resp: resp_tx,
            })
            .await
            .map_err(|e| {
                TingError::PluginExecutionError(format!("Failed to send init command: {}", e))
            })?;

        // Await response
        match resp_rx.await {
            Ok(res) => res,
            Err(_) => Err(TingError::PluginExecutionError(
    ...
    async fn shutdown(&self) -> Result<()> {
        ...
        self.tx
            .send(JsCommand::Shutdown { resp: resp_tx })
            .await
            .map_err(|e| {
                TingError::PluginExecutionError(format!("Failed to send shutdown command: {}", e))
            })?;
    ...
    async fn garbage_collect(&self) -> Result<()> {
        ...
        self.tx
            .send(JsCommand::GarbageCollect { resp: resp_tx })
            .await
            .map_err(|e| {
                TingError::PluginExecutionError(format!("Failed to send gc command: {}", e))
            })?;
...
impl JavaScriptPluginWrapper {
    pub async fn call_function(&self, name: &str, args: Value) -> Result<Value> {
        ...
        self.tx
            ...
            .await
            .map_err(|e| {
                TingError::PluginExecutionError(format!("Failed to send call command: {}", e))
            })?;

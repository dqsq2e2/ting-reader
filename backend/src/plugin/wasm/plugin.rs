...
impl WasmPlugin {
    ...
    pub async fn call(&self, function_name: &str, args: &[Val]) -> Result<Vec<Val>> {
        ...
        let func = instance
            ...
            .ok_or_else(|| {
                TingError::PluginExecutionError(format!("Function '{}' not found", function_name))
            })?;
        ...
        match call_result {
            ...
            Ok(Err(e)) => Err(TingError::PluginExecutionError(format!(
                "WASM function call failed: {}",
                e
            ))),
            Err(_) => Err(TingError::Timeout(format!(
                "WASM function call timed out: {}",
                function_name
            ))),
        }
    }
    ...
    pub async fn memory(&self) -> Result<Memory> {
        ...
        Err(TingError::PluginExecutionError(
            "Cannot access memory directly, use helper methods".to_string(),
        ))
    }
    ...
    pub async fn read_memory(&self, ptr: usize, len: usize) -> Result<Vec<u8>> {
        ...
        let memory = instance
            ...
            .ok_or_else(|| {
                TingError::PluginExecutionError("Memory export not found".to_string())
            })?;

        let mut buffer = vec![0u8; len];
        memory.read(&inner.store, ptr, &mut buffer).map_err(|e| {
            TingError::PluginExecutionError(format!("Failed to read memory: {}", e))
        })?;
        Ok(buffer)
    }
    ...
    pub async fn write_memory(&self, ptr: usize, data: &[u8]) -> Result<()> {
        ...
        memory.write(&mut inner.store, ptr, data).map_err(|e| {
            TingError::PluginExecutionError(format!("Failed to write memory: {}", e))
        })?;
        Ok(())
    }
    ...
    async fn alloc(&self, len: usize) -> Result<i32> {
        ...
        let instance = inner.instance;
        let func = instance
            .get_func(&mut inner.store, "alloc")
            .ok_or_else(|| {
                TingError::PluginExecutionError("Function 'alloc' not found".to_string())
            })?;

        let mut results = vec![Val::I32(0)];
        func.call_async(&mut inner.store, &[Val::I32(len as i32)], &mut results)
            .await
            .map_err(|e| TingError::PluginExecutionError(format!("Alloc failed: {}", e)))?;

        match results[0] {
            Val::I32(ptr) => Ok(ptr),
            _ => Err(TingError::PluginExecutionError(
                "Alloc returned non-i32".to_string(),
            )),
        }
    }
...
impl ScraperPlugin for WasmPlugin {
    ...
    async fn download_cover(&self, url: &str) -> Result<Vec<u8>> {
        ...
        if let Some(data_str) = wrapper.get("data").and_then(|v| v.as_str()) {
            ...
            base64::engine::general_purpose::STANDARD
                .decode(data_str)
                .map_err(|e| {
                    TingError::PluginExecutionError(format!("Invalid base64 cover: {}", e))
                })
        } else {
            Err(TingError::PluginExecutionError(
                "Invalid cover response".to_string(),
            ))
        }
    }
    ...
    async fn get_audio_url(&self, chapter_id: &str) -> Result<String> {
        ...
        wrapper
            ...
            .map(|s| s.to_string())
            .ok_or_else(|| {
                TingError::PluginExecutionError("Invalid audio url response".to_string())
            })
    }
}

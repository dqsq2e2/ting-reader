...
fn test_register_duplicate_plugin() {
    ...
    registry
        .register(metadata.clone(), instance.clone())
        .unwrap();
    let result = registry.register(metadata, instance);
...
fn test_get_all_dependencies_transitive() {
    ...
    middle_meta.dependencies.push(PluginDependency::new(
        "base".to_string(),
        "1.0.0".to_string(),
    ));
    ...
    top_meta.dependencies.push(PluginDependency::new(
        "middle".to_string(),
        "1.0.0".to_string(),
    ));
...
fn test_validate_dependency_graph_valid() {
    ...
    dep_meta.dependencies.push(PluginDependency::new(
        "base".to_string(),
        "1.0.0".to_string(),
    ));
    let (_, dep_inst) = create_test_plugin("dependent", "1.0.0");
...
fn test_circular_dependency_detection() {
    ...
    b_meta.dependencies.push(PluginDependency::new(
        "plugin-a".to_string(),
        "1.0.0".to_string(),
    ));
    ...
    c_meta.dependencies.push(PluginDependency::new(
        "plugin-b".to_string(),
        "1.0.0".to_string(),
    ));
...
fn test_load_order_respects_dependencies() {
    ...
    // Get load order
    let order = registry
        .get_load_order(&[top_id.clone(), middle_id.clone(), base_id.clone()])
        .unwrap();

...
pub async fn generate_regex(Json(req): Json<GenerateRegexRequest>) -> Result<impl IntoResponse> {
    ...
    if let Some(ref r) = num_range {
        ranges.push((r.clone(), r"(\d+)"));
    }
    if let Some(ref r) = title_range {
        ranges.push((r.clone(), r"(.+)"));
    }
    ...
    let (captured_index, captured_title) = if let Some(ref c) = caps {
        ...
        if let Some(ref r) = num_range {
            sorted_ranges.push((r.start, "index"));
        }
        if let Some(ref r) = title_range {
            sorted_ranges.push((r.start, "title"));
        }

...
async fn start_hls_transcoding(
    ...
    cmd.arg("-y")
        ...
        .arg("warning")
        .arg("-fflags")
        .arg("+genpts+igndts") // 快速启动标志
        .arg("-analyzeduration")
        .arg("1000000") // 减少分析时间到 1 秒
        .arg("-probesize")
        .arg("5000000"); // 减少探测大小到 5MB
    ...
    cmd.arg("-i")
        ...
        .arg("vod") // 明确指定为 VOD 类型
        .arg("-hls_segment_type")
        .arg("mpegts") // 明确指定分片类型
        .arg("-hls_flags")
        .arg("append_list+split_by_time") // 追加模式 + 精确时间分割

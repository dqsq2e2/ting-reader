...
pub async fn get_hls_playlist(
    ...
    Err(TingError::ExternalError("Playlist generation timeout".to_string()))
}
...
pub async fn get_hls_segment(
    ...
    for component in filename_path.components() {
        match component {
            ...
            _ => return Err(TingError::InvalidRequest("Path traversal detected".to_string())),
        }
    }
    
    // 只允许 .ts 和 .m3u8 文件
    let ext = filename_path.extension()
        .and_then(|e| e.to_str())
        .unwrap_or("");
    ...
    if !segment_path.starts_with(&temp_dir) {
        return Err(TingError::InvalidRequest("Path traversal detected".to_string()));
    }
    
    if !segment_path.exists() {
        return Err(TingError::NotFound(format!("Segment {} not found", filename)));
    }
...
pub async fn seek_hls_stream(
    ...
    // 获取会话数据
    let session_data = state.hls_session_manager.get_session_data(&session_id).await
        .ok_or_else(|| TingError::NotFound("Session not found".to_string()))?;

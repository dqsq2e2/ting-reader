...
fn test_validate_nfo() {
    ...
    let chapter_nfo_path = manager
        .write_chapter_nfo(123, 1, &chapter_metadata)
        .unwrap();
...
fn test_utf8_chapter_encoding() {
    ...
    assert!(read_metadata.source_url.as_ref().unwrap().contains("音频"));
...
fn test_xml_special_characters() {
    ...
    assert_eq!(read_metadata.title, "Book with <special> & \"characters\"");
    assert_eq!(
        read_metadata.intro,
